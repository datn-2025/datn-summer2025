...
>
  <option value="{{ url('/books') }}" {{ request()->segment(2) === null ? 'selected' : '' }}>All</option>
  @foreach($categories as $cat)
    <option value="{{ url('/books/' . $cat->id) }}" {{ request()->segment(2) == $cat->id ? 'selected' : '' }}>
      {{ $cat->name }}
    </option>
  @endforeach
</select>
          ...
          <div class="widget-product-authur pt-5">
            <div class="section-title overflow-hidden mb-2">
              <h3 class="d-flex flex-column mb-0">Authors</h3>
            </div>
            <ul class="product-tags mb-0 sidebar-list list-unstyled">
              @foreach ($authors as $author)
              <li class="tags-item">
                <a href="{{ url()->current() }}?authors={{ $author->id }}" 
                   class="{{ in_array($author->id, (array) request('authors', [])) ? 'active' : '' }}">
                  {{ $author->name }}
                </a>
              </li>
              @endforeach
          ...
          <div class="widget-product-brands pt-5">
            ...
            <ul class="product-tags mb-0 sidebar-list list-unstyled">
              ...
              <li class="tags-item">
                <a href="{{ url()->current() }}?brands={{ $brand->id }}"
                   class="{{ in_array($brand->id, (array) request('brands', [])) ? 'active' : '' }}">
                  {{ $brand->name }}
                </a>
              </li>
              @endforeach
