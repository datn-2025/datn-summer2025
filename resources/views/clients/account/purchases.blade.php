...
@section('account_content')
    ...
    <div id="reviewModal" class="fixed inset-0 z-50 hidden bg-black bg-opacity-40 flex items-center justify-center">
        <div class="bg-white shadow-lg w-full max-w-lg p-8 relative border border-black" style="border-radius:0;">
            <button type="button" class="absolute top-3 right-3 text-gray-400 hover:text-black text-2xl"
                onclick="closeReviewModal()">&times;</button>
            ...
            <form id="reviewForm" action="{{ route('account.review.store') }}" method="POST" class="space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4 mb-2">
                    <div><span class="block text-xs text-gray-500">Mã đơn hàng:</span><span id="modal_order_code"
                            class="font-semibold text-sm text-black"></span></div>
                    <div><span class="block text-xs text-gray-500">Trạng thái:</span><span id="modal_order_status"
                            class="font-semibold text-sm text-black"></span></div>
                    <div><span class="block text-xs text-gray-500">Vận chuyển:</span><span id="modal_shipping_method"
                            class="font-semibold text-sm text-black"></span></div>
                    <div><span class="block text-xs text-gray-500">Địa chỉ nhận:</span><span id="modal_shipping_address"
                            class="font-semibold text-sm text-black"></span></div>
                    <div><span class="block text-xs text-gray-500">Phương thức thanh toán:</span><span
                            id="modal_payment_method" class="font-semibold text-sm text-black"></span></div>
                    <div><span class="block text-xs text-gray-500">Thời gian đặt hàng:</span><span
                            id="modal_order_created_at" class="font-semibold text-sm text-black"></span></div>
                    <div><span class="block text-xs text-gray-500">Thời gian thanh toán:</span><span
                            id="modal_order_paid_at" class="font-semibold text-sm text-black"></span></div>
                    <div><span class="block text-xs text-gray-500">Thời gian hoàn thành:</span><span
                            id="modal_order_completed_at" class="font-semibold text-sm text-black"></span></div>
                </div>
                <div class="mb-2"><span class="block text-xs text-gray-500">Giá sản phẩm:</span><span
                        id="modal_book_price" class="font-semibold text-sm text-black"></span></div>
                <div class="mb-2"><span class="block text-xs text-gray-500">Số lượng:</span><span
                        id="modal_book_quantity" class="font-semibold text-sm text-black"></span></div>
                <div class="mb-2"><span class="block text-xs text-gray-500">Tác giả:</span><span id="modal_book_author"
                        class="font-semibold text-sm text-black"></span></div>
                <div class="mb-2"><span class="block text-xs text-gray-500">Nhà xuất bản/Brand:</span><span
                        id="modal_book_brand" class="font-semibold text-sm text-black"></span></div>
                <div class="mb-2"><span class="block text-xs text-gray-500">Tổng tiền hàng:</span><span
                        id="modal_order_subtotal" class="font-semibold text-sm text-black"></span></div>
                <div class="mb-2"><span class="block text-xs text-gray-500">Phí vận chuyển:</span><span
                        id="modal_order_shipping_fee" class="font-semibold text-sm text-black"></span></div>
                <div class="mb-2"><span class="block text-xs text-gray-500">Voucher giảm giá:</span><span
                        id="modal_order_voucher" class="font-semibold text-sm text-black"></span></div>
                <div class="mb-2"><span class="block text-xs text-gray-500">Thành tiền:</span><span
                        id="modal_order_total" class="font-semibold text-sm text-red-600"></span></div>
                <div>
                    ...
                    <div class="flex items-center space-x-1" id="modal_star_rating">
                        @for ($i = 5; $i >= 1; $i--)
                            <input type="radio" id="modal_star{{ $i }}" name="rating"
                                value="{{ $i }}" class="sr-only">
                            <label for="modal_star{{ $i }}"
                                class="cursor-pointer text-3xl text-slate-300 hover:text-yellow-400 transition-colors duration-150"
                                title="{{ $i }} sao">★</label>
                        @endfor
                    </div>
                </div>
    ...
    <div id="editReviewModal" class="fixed inset-0 z-50 hidden bg-black bg-opacity-40 flex items-center justify-center">
        <div class="bg-white shadow-lg w-full max-w-lg p-8 relative border border-black" style="border-radius:0;">
            <button type="button" class="absolute top-3 right-3 text-gray-400 hover:text-black text-2xl"
                onclick="closeEditReviewModal()">&times;</button>
            ...
            <form id="editReviewForm" action="" method="POST" class="space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4 mb-2">
                    <div><span class="block text-xs text-gray-500">Mã đơn hàng: {{ $item->order->order_code }}</span><span
                            id="edit_modal_order_code" class="font-semibold text-sm text-black"></span></div>
                    <div><span class="block text-xs text-gray-500">Trạng thái:
                            {{ $item->order->orderStatus->name }}</span><span id="edit_modal_order_status"
                            class="font-semibold text-sm text-black"></span></div>
                    <div><span class="block text-xs text-gray-500">Phí vận chuyển:
                            {{ $item->order->shipping_fee }}</span><span id="edit_modal_shipping_method"
                            class="font-semibold text-sm text-black"></span></div>
                    <div><span class="block text-xs text-gray-500">Địa chỉ nhận:
                            {{ $item->order->address->address_detail }}</span><span id="edit_modal_shipping_address"
                            class="font-semibold text-sm text-black"></span></div>
                    <div><span class="block text-xs text-gray-500">Phương thức thanh toán:
                            {{ $item->order->paymentMethod->name ?? 'Chưa xác định' }}</span><span
                            id="edit_modal_payment_method" class="font-semibold text-sm text-black"></span></div>
                    <div><span class="block text-xs text-gray-500">Thời gian đặt hàng:
                            {{ $item->order->created_at }}</span><span id="edit_modal_order_created_at"
                            class="font-semibold text-sm text-black"></span></div>
                </div>
                <div class="mb-2">
                ...
                <div class="space-y-2 text-sm">
                    <div class="flex justify-between">
                        ...
                        <span
                            class="text-gray-800 font-medium">{{ number_format($item->price * $item->quantity, 0, ',', '.') }}
                            đ</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-600">Phí vận chuyển:</span>
                        <span
                            class="text-gray-800 font-medium">{{ number_format($item->order->shipping_fee, 0, ',', '.') }}
                            đ</span>
                    </div>
                    @if ($item->order->discount_amount > 0)
                        <div class="flex justify-between text-green-600">
                            <span class="font-medium">Giảm giá
                                ({{ $item->order->applied_voucher_code ?? 'Voucher' }}):</span>
                            <span class="font-medium">- {{ number_format($item->order->discount_amount, 0, ',', '.') }}
                                đ</span>
                        </div>
                    @endif
                    ...
                    <div class="flex justify-between text-xl font-bold text-gray-900">
                        <span>Tổng cộng:</span>
                        <span>{{ number_format($item->order->total_amount, 0, ',', '.') }} đ</span>
                    </div>
                </div>
        </div>
        <div>
            ...
            <div class="flex items-center space-x-1" id="edit_modal_star_rating">
                @for ($i = 5; $i >= 1; $i--)
                    <input type="radio" id="edit_modal_star{{ $i }}" name="rating"
                        value="{{ $i }}" class="sr-only">
                    <label for="edit_modal_star{{ $i }}"
                        class="cursor-pointer text-3xl text-slate-300 hover:text-yellow-400 transition-colors duration-150"
                        title="{{ $i }} sao">★</label>
                @endfor
            </div>
        ...
        <div>
            <textarea name="comment" id="edit_modal_comment" rows="3"
                class="w-full px-3 py-2 border border-black rounded-none focus:ring-2 focus:ring-black focus:border-black transition-colors duration-200 text-sm resize-none text-black bg-white"
                placeholder="Nhận xét về sản phẩm..." value="{{ $item->comment }}"></textarea>
        </div>
        <button type="submit"
            class="w-full inline-flex items-center justify-center px-4 py-2 bg-black hover:bg-gray-900 text-white text-sm font-medium rounded-none transition-colors duration-200 focus:ring-2 focus:ring-black focus:ring-offset-2">
            Lưu đánh giá
