                    ...
                    <div class="row g-4 mb-3">
                        <div class="col-sm">
                            <form method="GET" action="{{ route('admin.categories.index') }}" class="d-flex justify-content-sm-end">
                                <input type="text" name="search_name_category" class="form-control me-2" placeholder="Tên danh mục" value="{{ $searchName ?? '' }}" style="width: 200px; height: 36px;">
                                ...
                                <tbody>
                                    @foreach ($categories as $key => $category)
                                        <tr>
                                            ...
                                            <td>{{ $category->name }}</td>
                                            <td>
                                                <img src="{{ $category->image ? asset('storage/' . $category->image) : asset('images/default-category.png') }}" alt="{{ $category->name }}" width="50">
                                            </td>
                                            ...
                                            <td>
                                                <div class="btn-group">
                                                    @if ($category->deleted_at)
                                                        ...
                                                        <form action="{{ route('admin.categories.force-delete', $category->id) }}" method="POST" onsubmit="return confirm('Bạn có chắc muốn xóa vĩnh viễn danh mục này?')" class="d-inline">
                                                            @csrf
                                                            @method('DELETE')
                                                    ...
                                                    @else
                                                        ...
                                                        <form action="{{ route('admin.categories.destroy', $category->id) }}" method="POST" onsubmit="return confirm('Bạn có chắc muốn xóa tạm thời danh mục này?')" class="d-inline">
                                                            @csrf
                                                            @method('DELETE')
